...
namespace Lykke.Job.IcoInvestment.Tests
{
    public class TokenPriceTests
    {
        ...
        {
            StartDateTimeUtc = DateTime.Today.ToUniversalTime(),
            EndDateTimeUtc = DateTime.Today.ToUniversalTime().AddDays(21),
            TokenDecimals = 4,
            TokenBasePriceUsd = 1M,
            TotalTokensAmount = 360_000_000
        };
        ...
        [InlineData(-1)]
        [InlineData(21)]
        public void ShouldReturnNullIfTransactionDateTimeIsOutOfTerms(int txDateTimeOffset)
        {
            // Arrange
            var currentTotal = 0M;
            var amountUsd = 1M;
            var txDateTimeUtc = DateTime.Now.ToUniversalTime().AddDays(txDateTimeOffset);
        ...
        [Fact]
        public void ShouldReturnNullIfAllTokensHadBeenSold()
        {
            // Arrange
            var currentTotal = 360_000_000M;
            var amountUsd = 1M;
            var txDateTimeUtc = DateTime.Now.ToUniversalTime();
